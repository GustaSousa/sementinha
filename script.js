// NAVBAR
    ...
    document.addEventListener('click', function(e) {
        ...
        if (!overlay.contains(e.target) && !document.getElementById('menu-toggle').contains(e.target)) {
    ...
    function animateNumbers() {
        ...
        numbers.forEach(number => {
            ...
            const interval = setInterval(() => {
                ...
                if (current >= target) {
                    clearInterval(interval);
                    number.textContent = target + (number.textContent.includes('%') ? '%' : ''); // Mantém o '%' se já estiver presente
                } else {
